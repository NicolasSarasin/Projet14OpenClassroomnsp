...
function EmployeeList() {
  ...
  // Pagination
  const totalEntries = filterEmployees.length;
      ...
      <div id='employee-table-filter' className='dataTables_Wrapper no-footer'>
        ...
        <table id="employee-table" className="display dataTable no-footer" role='grid' aria-describedby='employee-table_info'>
          <thead>
            <tr role='row' className='displayThead'>
              ...
              <th className='sorting' tabIndex="0" aria-controls='employee-table' rowSpan={"1"} colSpan={"1"} style={{width:"128px"}}>Last Name</th>
              ...
              <th className='sorting' tabIndex="0" aria-controls='employee-table' rowSpan={"1"} colSpan={"1"} style={{width:"249px"}}>Street</th>
              <th className='sorting' tabIndex="0" aria-controls='employee-table' rowSpan={"1"} colSpan={"1"} style={{width:"52px"}}>City</th>
        ...
        <div className='divFooter'>
          ...
          <div id='employee-table_paginate' className='dataTables_paginate paging_simple_numbers'>
            <a onClick={handlePrev} disabled={currentPage === 1} className="paginate-button previous disabled" aria-controls='employee-table' id='employee-table_previous'>Previous</a>
            <span>
              {pageNumbers().map((page,index) =>(
                <a key={index} 
                className={`paginate-button ${page === currentPage ? "current" : "notCurrent"}`} 
            ...
            <a onClick={handleNext} disabled={currentPage === totalPages || totalEntries === 0} className='paginate-button next disabled' aria-controls='employee-table' id='employee-table_next'>Next</a>
          </div>
        </div>
      </div>
      <Link to="/">Home</Link>
